...
const Page = ({ number, front, back, page, opened, bookClosed, totalPages, ...props }: PageProps) => {
    const [picture, picture2, pictureRoughness] = useTexture([
        ...
        ...(number === 0 || number === totalPages - 1
            ? [`/branding/textures/book-cover-roughness.jpg`]
            : []),
    ]);
    ...
    const manualSkinnedMesh = useMemo(() => {
        ...
        mesh.bind(skeleton);
        return mesh;
    }, [number, picture, picture2, pictureRoughness, totalPages]);
...
export const Book = ({ images, ...props }: { images: string[];[key: string]: any }) => {
    const [page] = useAtom(pageAtom);
    const [delayedPage, setDelayedPage] = useState(page);

    // Create page pairs from images array (front/back)
    // Odd number of images will duplicate the last image for the back
    ...
    return (
        <group {...props} rotation-y={-Math.PI / 2}>
            {formattedPages.map((pageData, index) => (
                <Page
                    key={index}
                    page={delayedPage}
                    number={index}
                    opened={delayedPage > index}
                    bookClosed={delayedPage === 0 || delayedPage === formattedPages.length}
